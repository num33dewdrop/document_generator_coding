...
module.exports = {
  ...
  entry: {
    common: path.resolve(__dirname, 'src/ts', 'common.ts'),
  },
  output: {
    path: path.resolve(__dirname, 'dist'),
    filename: 'js/bundle.js',
    assetModuleFilename: 'img/whole/[name]_[hash][ext]',
  },
  module: {
    rules: [
      {
  ...
  plugins: [
    ...
    new CopyWebpackPlugin({
      patterns: [
        {
          from: `${__dirname}/src/img/symbol/`,
          to: `${__dirname}/dist/img/symbol/`
        }
      ]
    }),
    ...
    ...glob.sync(path.resolve(__dirname, 'src/ejs/*.ejs'), { ignore: "_*.ejs", cwd: './src/ejs' }).map(file => new HtmlWebpackPlugin({
      template: file,
      filename: `${path.basename(file, '.ejs')}.html`,
      inject: 'body', // JSファイルをbodyの最後に挿入
      chunks: ['common'], // 挿入するチャンク
      minify: false // minifyを無効化（PHPファイルの場合）
    })),
  ],
  devServer: {
    static: {
